...
async fn main() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let version = esni.read_u16::<BigEndian>()?;

    let mut checksum = [0; 4];
    esni.read_exact(&mut checksum)?;
    let calculated_checksum = digest(&SHA256, esni);
    ...
    let keyshare_entries = KeyShareEntries::read(&mut reader).expect("keyshare");
    ...
    let ciphersuite = CipherSuite::read(&mut reader).expect("ciphersuite");

    let padded_lenght = esni.read_u16::<BigEndian>()?;
    let not_before = esni.read_u64::<BigEndian>()?;
    let not_after = esni.read_u64::<BigEndian>()?;

    let extension_size = esni.read_u16::<BigEndian>()?;
